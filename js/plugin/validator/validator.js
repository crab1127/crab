...
~function ($) {
  ...
  patterns = {
    ...
    text: function(text){
      ...
      if(false === notEmpty(text)){
        return false;
      }
      if(false === isNaN(max) && text.length > max){
        return false;
      }
      if(false === isNaN(min) && text.length < min){
        return false;
      }
      return true;
    }
  ...
  validate = function($item, klass, parent){
    ...
    // 去掉 validator 的 notEmpty 检测
    if(!(/^(?:radio|checkbox)$/.test(type) || aorb || $item.is('[validator]') ) && !patterns['text'](val))
  ...
  // 校验表单项
  validateFields = function($fields, method, klass, parent, liveCallback) {
    ...
    $.each($fields, function(i, f){
      $(f).on(reSpecialType.test(f.type) || "SELECT" === f.tagName ? 'change blur' : method, function(){
        ...
        $items.each(function(){
            ...
            if(field){
                liveCallback.call(this, field, false);
                unvalidFields.push(field);
            }
            else{
                field = { $el: $(this) };
                liveCallback.call(this, field, true);
            }
        });
      })
    })
  ...
  $.fn.validator = function(options) {
    var $form = this
      ...
      , liveCallback = options.liveCallback || function(field){}
    ...
    // 表单项校验
    method && validateFields.call(this, $items, method, klass, isErrorOnParent, liveCallback);

    // 当用户聚焦到某个表单时去除错误提示
    $form.off('focusin').on('focusin', identifie, function(e) {
